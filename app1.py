...
class DBProxy:
    def _get_db(self):
        db_actual = g.get('db_conn')
        if db_actual is None:
            db_actual = get_connection()
            g.db_conn = db_actual
        return db_actual

    def disponible(self):
        try:
            db_actual = self._get_db()
            return db_actual is not None and db_actual.is_connected()
        except Exception:
            return False
    ...
    def obtener_cursor(self):
        ...
        return db_actual.cursor(buffered=True)
...
def cerrar_conexion_db(_error=None):
    db_actual = g.pop('db_conn', None)
...
USUARIOS_TIENE_CAMPO_USUARIO = None


def usuarios_tiene_campo_usuario():
    global USUARIOS_TIENE_CAMPO_USUARIO

    if USUARIOS_TIENE_CAMPO_USUARIO is not None:
        return USUARIOS_TIENE_CAMPO_USUARIO
...
def test_db():
    ...
    return {'mensaje': 'Conectado a MySQL'}, 200
...
def listar_productos():
    if not db.disponible():
        return {'error': 'No hay conexi\\u00f3n a MySQL'}, 500
